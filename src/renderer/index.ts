...
import type { Theme, WrappedStats } from '../types.js';
...
export async function renderWrapped(
  ...
  const hasFeatures =
    featureUsage.topTools.length > 0 ||
  ...
  drawHeatmap(
    ctx, theme, RIGHT_X, bodyY, RIGHT_W, heatmapH,
    stats.dailyActivity, stats.period.to,
  );

  // Hourly chart
  drawHourlyChart(
    ctx, theme, RIGHT_X, hourlyY, RIGHT_W, hourlyH,
  ...
  if (hasTokenChart) {
    ...
    drawTokenChart(
      ctx, theme, PAD, tokenChartY + 14,
      CONTENT_W, tokenChartH - 14,
      stats.dailyTokenChart, stats.chartModelNames,
    );
  }
...
function buildFunFacts(stats: WrappedStats): FunFact[] {
  const facts: FunFact[] = [];
  ...
  const daysSinceFirst = Math.floor(
    (now.getTime() - firstDate.getTime()) / 86_400_000,
  );
  if (daysSinceFirst > 0) {
